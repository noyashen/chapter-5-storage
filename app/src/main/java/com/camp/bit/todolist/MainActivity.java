...
public class MainActivity extends AppCompatActivity {
    ...
    private List<Note> loadNotesFromDatabase() {
        ...
        Cursor cursor= null;
        try{
            cursor =database.query(TodoContract.TodoEntry.TABLE_NAME,null,null,null,null,null,null);
            while (cursor.moveToNext()){
                ...
                int intState = cursor.getInt(cursor.getColumnIndex(TodoContract.TodoEntry.COLUMN_SDATE));
    ...
    private void deleteNote(Note note) {
        ...
        int rows = database.delete(TodoContract.TodoEntry.TABLE_NAME, TodoContract.TodoEntry._ID+"=?",new String[]{String.valueOf(note.id)});
        if(rows>0){
            notesAdapter.refresh(loadNotesFromDatabase());
        }
    }
    ...
    private void updateNode(Note note) {
        ...
        values.put(TodoContract.TodoEntry.COLUMN_SDATE,note.getState().intValue);
